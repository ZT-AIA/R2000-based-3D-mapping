...
#line 4 "..\\..\\libraries\\ptpd-2.0.0\\src\\dep\\net.c"
...
static _Bool netQPut(BufQueue *queue, void *pbuf)
{
	_Bool retval = 0;
	...
	if (((queue->head + 1) & (4 - 1)) != queue->tail)
	{
		
		queue->head = (queue->head + 1) & (4 - 1);
...
static _Bool netQCheck(BufQueue  *queue)
{
	_Bool  retval = 0;

	sys_mutex_lock(&queue->mutex);

	if (queue->tail != queue->head) retval = 1;
...
_Bool netShutdown(NetPath *netPath)
{
	...
	igmp_leavegroup(((ip_addr_t *)&ip_addr_any), &multicastAaddr);
...
_Bool netInit(NetPath *netPath, PtpClock *ptpClock)
{
	...
	memcpy(addrStr, "224.0.1.129", 16);
	if (!ipaddr_aton(addrStr, (ip_addr_t*)&netAddr))
	{
			;
			goto fail04;
	}
	...
	udp_bind(netPath->eventPcb, ((ip_addr_t *)&ip_addr_any), 319);
	 

	 
	udp_recv(netPath->generalPcb, netRecvGeneralCallback, netPath);
	udp_bind(netPath->generalPcb, ((ip_addr_t *)&ip_addr_any), 320);
...
static ssize_t netRecv(octet_t *buf, TimeInternal *time, BufQueue *msgQueue)
{
	...
	if ((p = (struct pbuf*) netQGet(msgQueue)) == 0)
	{
		return 0;
	}
...
static ssize_t netSend(const octet_t *buf, int16_t  length, TimeInternal *time, const int32_t * addr, struct udp_pcb * pcb)
{
	...
	if (time != 0)
	{
		...
		time->seconds = p->time_sec;
		time->nanoseconds = p->time_nsec;
#line 392 "..\\..\\libraries\\ptpd-2.0.0\\src\\dep\\net.c"
		;
	} else {
		;
	}
...
{
	return netSend(buf, length, 0, &netPath->multicastAddr, netPath->generalPcb);
}

ssize_t netSendPeerGeneral(NetPath *netPath, const octet_t *buf, int16_t  length)
{
	return netSend(buf, length, 0, &netPath->peerMulticastAddr, netPath->generalPcb);
}
