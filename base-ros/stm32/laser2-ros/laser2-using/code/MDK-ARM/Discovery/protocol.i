...
#line 4 "..\\..\\libraries\\ptpd-2.0.0\\src\\protocol.c"

static void handle(PtpClock*);
static void handleAnnounce(PtpClock*, _Bool);
static void handleSync(PtpClock*, TimeInternal*, _Bool);
static void handleFollowUp(PtpClock*, _Bool);
static void handlePDelayReq(PtpClock*, TimeInternal*, _Bool);
static void handleDelayReq(PtpClock*, TimeInternal*, _Bool);
static void handlePDelayResp(PtpClock*, TimeInternal*, _Bool);
static void handleDelayResp(PtpClock*, _Bool);
static void handlePDelayRespFollowUp(PtpClock*, _Bool);
static void handleManagement(PtpClock*, _Bool);
static void handleSignaling(PtpClock*, _Bool);
...
static _Bool doInit(PtpClock*);

#line 49 "..\\..\\libraries\\ptpd-2.0.0\\src\\protocol.c"

 
void toState(PtpClock *ptpClock, uint8_t state)
{
	ptpClock->messageActivity = 1;
	...
	switch (state)
	{
		...
		case PTP_LISTENING:
			...
			timerStart(ANNOUNCE_RECEIPT_TIMER, (ptpClock->portDS.announceReceiptTimeout) * ((((ptpClock->portDS . logAnnounceInterval)>0) ? (1000 << (ptpClock->portDS . logAnnounceInterval)) : (1000 >>(-(ptpClock->portDS . logAnnounceInterval))))));
		...
		case PTP_MASTER:
			...
			timerStart(SYNC_INTERVAL_TIMER, (((ptpClock->portDS . logSyncInterval)>0) ? (1000 << (ptpClock->portDS . logSyncInterval)) : (1000 >>(-(ptpClock->portDS . logSyncInterval)))));
			;
			timerStart(ANNOUNCE_INTERVAL_TIMER, (((ptpClock->portDS . logAnnounceInterval)>0) ? (1000 << (ptpClock->portDS . logAnnounceInterval)) : (1000 >>(-(ptpClock->portDS . logAnnounceInterval)))));
			...
			switch (ptpClock->portDS.delayMechanism)
			{
				case E2E:
						...
						break;
				case P2P:
						timerStart(PDELAYREQ_INTERVAL_TIMER, getRand((((ptpClock->portDS . logMinPdelayReqInterval)>0) ? (1000 << (ptpClock->portDS . logMinPdelayReqInterval)) : (1000 >>(-(ptpClock->portDS . logMinPdelayReqInterval)))) + 1));
						break;
				default:
						break;
			}
		...
		case PTP_PASSIVE:
			...
			{
				timerStart(PDELAYREQ_INTERVAL_TIMER, getRand((((ptpClock->portDS . logMinPdelayReqInterval + 1)>0) ? (1000 << (ptpClock->portDS . logMinPdelayReqInterval + 1)) : (1000 >>(-(ptpClock->portDS . logMinPdelayReqInterval + 1))))));
			}
		...
		case PTP_UNCALIBRATED:
			...
			{
				case E2E:
						timerStart(DELAYREQ_INTERVAL_TIMER, getRand((((ptpClock->portDS . logMinDelayReqInterval + 1)>0) ? (1000 << (ptpClock->portDS . logMinDelayReqInterval + 1)) : (1000 >>(-(ptpClock->portDS . logMinDelayReqInterval + 1))))));
						break;
				case P2P:
						timerStart(PDELAYREQ_INTERVAL_TIMER, getRand((((ptpClock->portDS . logMinPdelayReqInterval + 1)>0) ? (1000 << (ptpClock->portDS . logMinPdelayReqInterval + 1)) : (1000 >>(-(ptpClock->portDS . logMinPdelayReqInterval + 1))))));
						break;
...
static _Bool doInit(PtpClock *ptpClock)
{
	;
...
void doState(PtpClock *ptpClock)
{
	...
	switch (ptpClock->portDS.portState)
	{
		...
		case PTP_PASSIVE:
			...
			if ((_Bool)(((ptpClock->events) & (STATE_DECISION_EVENT)) == (STATE_DECISION_EVENT)))
			{
				;
				(ptpClock->events) &= ~(STATE_DECISION_EVENT);
		...
		case PTP_SLAVE:
			...
			switch (ptpClock->portDS.portState)
			{
				case PTP_UNCALIBRATED:
					...
					if ((_Bool)(((ptpClock->events) & (MASTER_CLOCK_SELECTED)) == (MASTER_CLOCK_SELECTED)))
					{
						;
						(ptpClock->events) &= ~(MASTER_CLOCK_SELECTED);
						toState(ptpClock, PTP_SLAVE);
					}

					if ((_Bool)(((ptpClock->events) & (MASTER_CLOCK_CHANGED)) == (MASTER_CLOCK_CHANGED)))
					{
						;
				...
				case PTP_SLAVE:
					...
					if ((_Bool)(((ptpClock->events) & (SYNCHRONIZATION_FAULT)) == (SYNCHRONIZATION_FAULT)))
					{
							;
							(ptpClock->events) &= ~(SYNCHRONIZATION_FAULT);
...
static void handleAnnounce(PtpClock *ptpClock, _Bool isFromSelf)
{
	_Bool  isFromCurrentParent = 0;

	;

	if (ptpClock->msgIbufLength < 64)
	{
			;
			toState(ptpClock, PTP_FAULTY);
			return;
...
static void handleSync(PtpClock *ptpClock, TimeInternal *time, _Bool isFromSelf)
{
	...
	switch (ptpClock->portDS.portState)
	{
		...
		case PTP_SLAVE:
			...
			if ((_Bool)(((ptpClock->msgTmpHeader . flagField[0]) & (FLAG0_TWO_STEP)) == (FLAG0_TWO_STEP)))
			{
				ptpClock->waitingForFollowUp = 1;
...
static void handleFollowUp(PtpClock *ptpClock, _Bool isFromSelf)
{
...
static void handleDelayReq(PtpClock *ptpClock, TimeInternal *time, _Bool isFromSelf)
{
	switch (ptpClock->portDS.delayMechanism)
	{
		case E2E:
...
static void handleDelayResp(PtpClock *ptpClock, _Bool  isFromSelf)
{
	_Bool  isFromCurrentParent = 0;
	_Bool  isCurrentRequest = 0;
	TimeInternal correctionField;
...
static void handlePDelayReq(PtpClock *ptpClock, TimeInternal *time, _Bool  isFromSelf)
{
	switch (ptpClock->portDS.delayMechanism)
	{
		case E2E:
		...
		case P2P:
			...
			switch (ptpClock->portDS.portState)
			{
				...
				case PTP_MASTER:
					...
					if ((time->seconds != 0) && (_Bool)(((ptpClock->msgTmpHeader . flagField[0]) & (FLAG0_TWO_STEP)) == (FLAG0_TWO_STEP)))  
					{
...
static void handlePDelayResp(PtpClock *ptpClock, TimeInternal *time, _Bool isFromSelf)
{
	TimeInternal requestReceiptTimestamp;
	TimeInternal correctionField;
	_Bool  isCurrentRequest;
			...
			switch (ptpClock->portDS.portState)
			{
				...
				case PTP_SLAVE:
					...
					if (((ptpClock->sentPDelayReqSequenceId - 1) == ptpClock->msgTmpHeader.sequenceId) && isCurrentRequest)
					{
						if ((_Bool)(((ptpClock->msgTmpHeader . flagField[0]) & (FLAG0_TWO_STEP)) == (FLAG0_TWO_STEP)))
						{
							ptpClock->waitingForPDelayRespFollowUp = 1;
						...
						{
							ptpClock->waitingForPDelayRespFollowUp = 0;

							 
							ptpClock->pdelay_t4 = *time;

							scaledNanosecondsToInternalTime(&ptpClock->msgTmpHeader.correctionfield, &correctionField);
							updatePeerDelay(ptpClock, &correctionField, 0);
						}
					}
					else
					{
...
static void handlePDelayRespFollowUp(PtpClock *ptpClock, _Bool isFromSelf)
{
			...
			switch (ptpClock->portDS.portState)
			{
				...
				case PTP_MASTER:
					...
					if (ptpClock->msgTmpHeader.sequenceId == ptpClock->sentPDelayReqSequenceId - 1)
					{
							...
							updatePeerDelay(ptpClock, &correctionField, 1);
							ptpClock->waitingForPDelayRespFollowUp = 0;
							break;
					}
...
static void handleManagement(PtpClock *ptpClock, _Bool isFromSelf)
{
	 
	 
}

static void handleSignaling(PtpClock *ptpClock, _Bool  isFromSelf)
{
}
...
static void issueAnnounce(PtpClock *ptpClock)
{
	...
	if (!netSendGeneral(&ptpClock->netPath, ptpClock->msgObuf, 64))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
...
static void issueSync(PtpClock *ptpClock)
{
	...
	if (!netSendEvent(&ptpClock->netPath, ptpClock->msgObuf, 44, &internalTime))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
...
static void issueFollowup(PtpClock *ptpClock, const TimeInternal *time)
{
	...
	if (!netSendGeneral(&ptpClock->netPath, ptpClock->msgObuf, 44))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
...
static void issuePDelayReq(PtpClock *ptpClock)
{
	...
	if (!netSendPeerEvent(&ptpClock->netPath, ptpClock->msgObuf, 54, &internalTime))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
...
static void issuePDelayResp(PtpClock *ptpClock, TimeInternal *time, const MsgHeader * pDelayReqHeader)
{
	...
	if (!netSendPeerEvent(&ptpClock->netPath, ptpClock->msgObuf, 54, time))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
...
static void issueDelayResp(PtpClock *ptpClock, const TimeInternal *time, const MsgHeader * delayReqHeader)
{
	...
	if (!netSendGeneral(&ptpClock->netPath, ptpClock->msgObuf, 54))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
...
static void issuePDelayRespFollowUp(PtpClock *ptpClock, const TimeInternal *time, const MsgHeader * pDelayReqHeader)
{
	...
	if (!netSendPeerGeneral(&ptpClock->netPath, ptpClock->msgObuf, 54))
	{
		;
		toState(ptpClock, PTP_FAULTY);
	}
