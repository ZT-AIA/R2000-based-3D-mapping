...
void navudp_callback(void *arg,struct udp_pcb *pcb,struct pbuf *p,ip_addr_t *addr,u16_t port)
{
	...
	if(netpath->front!=((netpath->tail+1)&(8-1)))
	{
	...
	sys_mbox_trypost(&navReceiveQue,0);
}
...
void handlerRecdata(struct pbuf *p,struct net_path *netpath)
{
	...
	else
	{
		...
		{
			recv_msg.type_dscr = MSG_ACK;
			sysstatus.hostready=1;
		}
		else if(recv_msg_type==END_TRANS)
		{
			recv_msg.type_dscr = MSG_ACK;
			sysstatus.hostready = 0;
		}
		else
...
stat_set hybrid_nav_msg_send(struct net_path *net, struct pbuf *p)
{
	...
	if(HNMsg_Buf.front != HNMsg_Buf.rear)
	{
		...
		rt_tsk_lock();
		HNMsg_Buf.front = (HNMsg_Buf.front + 1)%3;
		rt_tsk_unlock();
...
void NavgationProcess_Thread(void *msg)
{
	...
	net.udppcb = udp_new();
	net.remoteport = 10000;
	net.localport = 10000;
	(&(net . remoteip_addr))->addr = ((u32_t)((100) & 0xff) << 24) | ((u32_t)((1) & 0xff) << 16) | ((u32_t)((168) & 0xff) << 8) | (u32_t)((192) & 0xff);
	...
	udp_bind(net.udppcb,((ip_addr_t *)&ip_addr_any),10000);
	for(;;)
	{
		while(net.tail!=net.front)
		{
			...
			net.front = (net.front+1)&(8-1);
			sys_mutex_unlock(&(net.bufmutex));
			rt_tsk_unlock();
			handlerRecdata(p,&net);
			pbuf_free(p);
		}
		if(sysstatus.hostready==1)
		{
			hybrid_nav_msg_send(&net,p_hybrid_nav);
		}
		sys_arch_mbox_fetch(&navReceiveQue,0,10);
	}
}
...
void NavgationProcess_Init(void)
{
	...
	sys_thread_new("NavgationProcess_Thread",NavgationProcess_Thread,0,1024*2,osPriorityNormal);
}
