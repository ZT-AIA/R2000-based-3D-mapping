...
#line 2 "..\\..\\libraries\\ptpd-2.0.0\\src\\dep\\servo.c"
...
void initClock(PtpClock *ptpClock)
{
	...
	ptpClock->parentDS.parentStats = 0;
									...
									const TimeInternal *preciseOriginTimestamp, const TimeInternal *correctionField)
{
	...
	if (abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) < 10000)
	{
	...
	else if (abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) > 1000000)
	{
		if (ptpClock->portDS.portState == PTP_SLAVE)
		{
				(ptpClock->events) |= (SYNCHRONIZATION_FAULT);
...
void updatePeerDelay(PtpClock *ptpClock, const TimeInternal *correctionField, _Bool  twoStep)
{
	;
...
void updateClock(PtpClock *ptpClock)
{
	...
	if (ptpClock->currentDS.offsetFromMaster.seconds != 0 || abs(ptpClock->currentDS.offsetFromMaster.nanoseconds) > 100000000)
	{
		...
		if (!ptpClock->servo.noAdjust)
		{
			...
			{
				adj = ptpClock->currentDS.offsetFromMaster.nanoseconds > 0 ? 512000 : -512000;
				adjFreq(-adj);
			}
		}
	}
	else
	{
		...
		if (ptpClock->observedDrift > 512000)
			ptpClock->observedDrift = 512000;
		else if (ptpClock->observedDrift < -512000)
			ptpClock->observedDrift = -512000;
		...
		if (0)
		{
			...
			ptpClock->parentDS.parentStats = 1;
