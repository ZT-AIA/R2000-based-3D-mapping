...
#line 4 "..\\..\\libraries\\ptpd-2.0.0\\src\\dep\\timer.c"

 
static sys_timer_t ptpdTimers[TIMER_ARRAY_SIZE];
static _Bool ptpdTimersExpired[TIMER_ARRAY_SIZE];
...
_Bool timerExpired(int32_t index)
{
	 
	if (index >= TIMER_ARRAY_SIZE) return 0;

	 
	if (!ptpdTimersExpired[index]) return 0;
	;
	ptpdTimersExpired[index] = 0;
