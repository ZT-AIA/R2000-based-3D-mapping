...
typedef _Bool (*shell_func)(int argc, char **argv);
...
static _Bool shell_exit(int argc, char **argv);
static _Bool shell_help(int argc, char **argv);
static _Bool shell_date(int argc, char **argv);
static _Bool shell_ptpd(int argc, char **argv);
...
static int shell_parse(char **str, char **word)
{
	...
	while (**str && ((* __rt_ctype_table())[* *str] & 1)) (*str)++;
...
static _Bool shell_command(char *cmdline)
{
	int i;
	char *argv[8];
	int argc = 0;
...
void shell_process() 
{
  ...
  while (telnet_gets(cmdbuf, sizeof(cmdbuf), 0, 1))
  {
