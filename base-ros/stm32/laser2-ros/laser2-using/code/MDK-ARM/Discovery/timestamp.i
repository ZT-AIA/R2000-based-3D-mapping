...
struct msgtime{
	struct msghead head;
	struct timeoffset offset[10];
};

typedef struct msg_timeoffset{
	struct msgtime time[4];
	uint8_t index;
	uint8_t head;
	uint8_t tail;
	sys_mutex_t mutex;
...
#line 8 "..\\src\\timeStamp.c"
#line 9 "..\\src\\timeStamp.c"
...
{
	sys_mbox_trypost(&rec_queue_time,0);
}

void time_queue_fecth(void)
{
	sys_arch_mbox_fetch(&rec_queue_time,0,100);
}
...
void set_timeoffset(struct timeoffset *time)
{
	...
	time->to_sec = ptpClock.currentDS.offsetFromMaster.seconds;
	time->to_nsec = ptpClock.currentDS.offsetFromMaster.nanoseconds;
}
...
void datacollection_thread(void *msg)
{
	...
	for(;;)
	{
		...
		if(((TIME_TEST.tail+1)&(4-1))!=TIME_TEST.tail)
		{
			if(TIME_TEST.index==10)
			{
				rt_tsk_lock();
				TIME_TEST.index = 0;
				TIME_TEST.tail = (TIME_TEST.tail+1)&(4-1);
				rt_tsk_unlock();
			}
...
void timetest_callback(void *arg,struct udp_pcb *pcb,struct pbuf *p,ip_addr_t *addr,u16_t port)
{
	...
	if((netPath->tail+1)%4!=netPath->head)
	{
		(netPath->bufque)[netPath->tail] = p;
		netPath->tail = (netPath->tail+1)&(4-1);
		netPath->remoteip = *addr;
...
void handletime(struct pbuf *p,net_Path *netpath)
{
	...
	switch(head.msg_type)
	{
		...
		case hostRead:
			...
			udp_sendto(netpath->udppcb,p,&(netpath->remoteip),netpath->remoteport);
...
void time_thread(void *mag)
{
	...
	udp_bind(netPath.udppcb,((ip_addr_t *)&ip_addr_any),netPath.port);
	
	
	p = pbuf_alloc(PBUF_TRANSPORT,sizeof(struct msgtime),PBUF_ROM);
	
	sys_thread_new("datacollection_thread",datacollection_thread,0,1024,osPriorityBelowNormal);
	
	for(;;)
	{
		while(netPath.head!=netPath.tail)
		{
			
			sys_mutex_lock(&(netPath.bufque_mutex));
			msgbuf = netPath.bufque[netPath.head];
			netPath.head = (netPath.head+1)%4;
			sys_mutex_unlock(&(netPath.bufque_mutex));
		...
		{
			if(TIME_TEST.head!=TIME_TEST.tail)
			{
				...
				TIME_TEST.head = (TIME_TEST.head+1)&(4-1);
		...
		else if(timestate.devreset==TOK&&dataflag==resetstate)
		{
			...
			last_pbuf = pbuf_alloc(PBUF_TRANSPORT,sizeof(struct msghead)+				sizeof(struct timeoffset)*TIME_TEST.index,PBUF_POOL);

			pbuf_take(last_pbuf,&(TIME_TEST.time[TIME_TEST.head]),			sizeof(struct msghead)+sizeof(struct timeoffset)*TIME_TEST.index);
...
void time_init(void)
{
	...
	sys_thread_new("time_thread",time_thread,0,1024*2,osPriorityAboveNormal);
}
