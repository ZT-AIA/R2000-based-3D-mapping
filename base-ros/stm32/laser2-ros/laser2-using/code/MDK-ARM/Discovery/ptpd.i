...
#line 4 "..\\..\\libraries\\ptpd-2.0.0\\src\\ptpd.c"
...
RunTimeOpts rtOpts;
PtpClock ptpClock;
ForeignMasterRecord ptpForeignRecords[5];
...
static void ptpd_thread(void *arg)
{
	...
	rtOpts.announceInterval = 1;
	rtOpts.syncInterval = 0;
	rtOpts.clockQuality.clockAccuracy = 0xFE;
	rtOpts.clockQuality.clockClass = 248;
	rtOpts.clockQuality.offsetScaledLogVariance = 5000;  
	rtOpts.priority1 = 248;
	rtOpts.priority2 = 248;
	rtOpts.domainNumber = 0;
	rtOpts.slaveOnly = 1;
	rtOpts.currentUtcOffset = 34;
	rtOpts.servo.noResetClock = 0;
	rtOpts.servo.noAdjust = 0;
	rtOpts.inboundLatency.nanoseconds = 0;
	rtOpts.outboundLatency.nanoseconds = 0;
	rtOpts.servo.sDelay = 6;
	rtOpts.servo.sOffset = 1;
	rtOpts.servo.ap = 2;
	rtOpts.servo.ai = 16;
...
#line 58 "..\\..\\libraries\\ptpd-2.0.0\\src\\ptpd.c"
...
void ptpd_alert(void)
{
	...
	sys_mbox_trypost(&ptp_alert_queue, 0);
}

void ptpd_init(void)
{
	
  if (sys_mbox_new(&ptp_alert_queue, 8) != 0)
	{
    printf("PTPD: failed to create ptp_alert_queue mbox");
  }

	
	sys_thread_new("PTPD", ptpd_thread, 0, 1024 * 2, osPriorityAboveNormal);
}
