...
#line 47 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 48 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 49 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 50 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 51 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 52 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 53 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
#line 54 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
#line 55 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 56 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 58 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 59 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
#line 60 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
...
void ETH_PTPTime_GetTime(struct ptptime_t * timestamp)
{
  timestamp->tv_nsec = ETH_PTPSubSecond2NanoSecond(ETH_GetPTPRegister(((uint32_t)0x0000070C)));
  timestamp->tv_sec = ETH_GetPTPRegister(((uint32_t)0x00000708));
}
...
static void low_level_init(struct netif *netif)
{
  ...
  ETH_PTPStart(((uint32_t)0x00000001));
   

  
   
	sys_thread_new((const char *) "Eth_if", ethernetif_input, 0, (350), (osPriorityRealtime));
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  u8 *buffer ;
	...
	if (sys_arch_sem_wait(&s_xTxSemaphore, (250)))
  {
    ...
    for (q = p; q != 0; q = q->next) 
    {
      ...
      l = l + q->len;
		...
		if (ETH_Prepare_Transmit_Descriptors_TimeStamp(l, &timeStamp) != ((uint32_t)1))
		{
			retval = -15;
		}
		else
...
#line 261 "..\\..\\libraries\\lwip-1.4.1\\port\\STM32F4x7\\arch\\ethernetif.c"
  ...
  if ((frame.descriptor->Status & ((uint32_t)0x00008000)) == (uint32_t)RESET)
  {
    ...
    { 
			...
			for(q = p; q != 0; q = q->next)
			{
				...
				while( (byteslefttocopy + bufferoffset) > 1524 )
				{
					 
					memcpy( (u8_t*)((u8_t*)q->payload + payloadoffset), (u8_t*)((u8_t*)buffer + bufferoffset), (1524 - bufferoffset));

					 
					DMARxNextDesc = (ETH_DMADESCTypeDef *)(DMARxNextDesc->Buffer2NextDescAddr);
					buffer = (unsigned char *)(DMARxNextDesc->Buffer1Addr);

					byteslefttocopy = byteslefttocopy - (1524 - bufferoffset);
					payloadoffset = payloadoffset + (1524 - bufferoffset);
					bufferoffset = 0;
				}
...
void ethernetif_input(void * pvParameters)
{
  ...
  for( ;; )
  {
		if (sys_arch_sem_wait(&s_xRxSemaphore, ((u32_t) 100)) != 0xffffffffUL)
    {
			do
			{
				...
				if (p != 0)
				{
					if (s_pxNetIf->input(p, s_pxNetIf) != 0)
					{
						pbuf_free(p);
						p=0;
					}
...
static void ETH_PTPStart(uint32_t UpdateMethod)
{
  ...
  ETH_MACITConfig(((uint32_t)0x00000200), DISABLE);

   
  ETH_PTPTimeStampCmd(ENABLE);

   
  ETH_SetPTPSubSecondIncrement(43);  

  if (UpdateMethod == ((uint32_t)0x00000001))
	{
    
 
    ETH_SetPTPTimeStampAddend(0x4C19EF00);
    ETH_EnablePTPTimeStampAddend();

     
    while(ETH_GetPTPFlagStatus(((uint32_t)0x00000020)) == SET);
  }
  ...
  ETH_SetPTPTimeStampUpdate(((uint32_t)0x00000000), 0, 0);
...
void ETH_PTPTime_AdjFreq(int32_t Adj)
{
	...
	addend = ((((275LL * Adj)>>8) * (0x4C19EF00>>24))>>6) + 0x4C19EF00;
...
void ETH_PTPTime_UpdateOffset(struct ptptime_t * timeoffset)
{
	...
	addend = ETH_GetPTPRegister(((uint32_t)0x00000718));

	while(ETH_GetPTPFlagStatus(((uint32_t)0x00000008)) == SET);
	while(ETH_GetPTPFlagStatus(((uint32_t)0x00000004)) == SET);
